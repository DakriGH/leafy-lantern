...
import { CHUNK } from '../world/world.js?v=mtbvpm5v';
import { elencoLuci, statLuci, statImpatti, memoriaVoxel } from '../fx/materials.js?v=mtbvpm5v';
import { FISICA } from '../config.js?v=mtbvpm5v';
...
export class MenuDebug {
  ...
  sincronizza() {
    ...
    if (c && this.azioni.controluceStato) {
      // ⚠ TRE STATI, quindi il bottone SCRIVE quale, non si limita ad accendersi:
      // con tre sistemi un bottone acceso/spento non basta più a dire cosa si sta
      // guardando — ed è già successo di giudicare un sistema per l'altro.
      const q = this.azioni.controluceStato();
      const nomi = { controluce: '🌓 Controluce', marcia: '🧭 Marcia voxel', campo: '🗺 Campo (vecchio)' };
  ...
  _costruisciManopole() {
    ...
    this._manopole = [
      ...
      { id: 'affila', nome: '✂️ Nitidezza del bordo', min: 0, max: 30, passo: 1, unita: '/10 px',
        ...
        scrivi: (v) => { if (this.azioni.affilaOmbra) this.azioni.affilaOmbra(v / 10); } },
