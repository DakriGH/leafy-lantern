...
import { MEZZO_SUPER } from '../config.js?v=mtbvpm5v';
import { convertiUnlit, materialiConMappa, patchLuci } from '../fx/materials.js?v=mtbvpm5v';
...
function fallback(defId) {
  ...
  } else if (defId === 'ciuffo' || defId === 'petali' || defId === 'foglieSecche') {
    // ⚠ NIENTE MODELLO, DI PROPOSITO. Erba, petali e foglie secche esistono già
    // come sistema instanziato (fx/erba.js, fx/foglie.js): sagoma di foglia
    // ritagliata nello shader, calpestio che le fa volare via, vento, ombra del
    // sole, luci dei lampioni. La prima versione dell'oggetto piazzabile si era
    // costruita un modellino tutto suo — «un furni versione brutta delle
    // particelle», e il committente l'ha visto al primo sguardo. Aveva ragione
    // due volte: era brutto, e soprattutto erano DUE sistemi che disegnano la
    // stessa cosa in due modi, cioè due estetiche destinate a divergere.
    //
    // Adesso il furni è un SEGNAPOSTO: serve a piazzare, a occupare la cella, a
    // salvarsi col diorama e a farsi togliere. A disegnarlo è il sistema vero,
    // che main avvisa piazzandolo (erba.posa / foglie.posa).
